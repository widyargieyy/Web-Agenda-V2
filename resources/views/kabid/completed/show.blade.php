    ...
    <div class="page-title-head d-flex align-items-center">
        ...
        <div class="text-end">
            <ol class="breadcrumb m-0 py-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('kabid.approval.index') }}">Data Agenda</a></li>
...
@section('content')
                                ...
                                <div
                                    ...
                                    <div class="d-flex gap-2 ms-3 flex-shrink-0">
                                        ...
                                        <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                            ...
                                            <i class="ti ti-x me-1"></i>Batalkan Agenda
                                        </button>
                                        {{-- Tombol Setujui --}}
                                        <form action="{{ route('kabid.completed.complete', $data->id) }}" method="POST"
                                            id="form-approve">
                                            @csrf
                                            @method('PATCH')
                                            <button type="button" class="btn btn-sm btn-success"
                                                onclick="confirmApprove()">
                                                <i class="ti ti-check me-1"></i>Selesaikan Agenda
                                            </button>
                                        </form>
                                    </div>
                                </div>
    ...
    {{-- ===================== MODAL TOLAK AGENDA ===================== --}}
    @if ($data->status == 'APPROVED')
        <div class="modal fade" id="modal-tolak" tabindex="-1" aria-labelledby="modal-tolak-label" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-header border-bottom border-dashed pb-3">
                        <div class="d-flex align-items-center gap-2">
                            <div
                                class="avatar-sm bg-danger-subtle rounded-2 d-flex align-items-center justify-content-center">
                                <i class="ti ti-x text-danger fs-lg"></i>
                            </div>
                            <div>
                                <h5 class="modal-title mb-0 fw-semibold" id="modal-tolak-label">Batalkan Agenda</h5>
                                <p class="text-muted fs-xs mb-0">Konfirmasi pembatalan agenda</p>
                            </div>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>

                    <form action="{{ route('kabid.completed.cancelled', $data->id) }}" method="POST" id="form-reject">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="modal-body py-4">
                            ...
                            <div class="d-flex align-items-center gap-2 p-3 bg-light bg-opacity-75 rounded-2 mb-4">
                                ...
                                <div class="overflow-hidden">
                                    <p class="fw-semibold fs-sm mb-0 text-truncate">{{ $data->title ?? '-' }}</p>
                            ...
                            {{-- <div class="mb-1">
                                ...
                                <textarea class="form-control" id="rejection-reason" name="rejected_reason" rows="4"
                                    placeholder="Tuliskan alasan penolakan secara jelas agar operator dapat melakukan perbaikan..."
                                    style="resize: none;" required maxlength="500" oninput="updateCharCount(this)"></textarea>
                                <div class="d-flex justify-content-between mt-1">
                                    <p class="text-muted fs-xs mb-0">Minimal 10 karakter</p>
                                    <p class="text-muted fs-xs mb-0"><span id="char-count">0</span>/500</p>
                                </div>
                            </div> --}}
                        ...
                        <div class="modal-footer border-top border-dashed pt-3">
                            ...
                            <button type="submit" class="btn btn-danger btn-sm">
                                <i class="ti ti-x me-1"></i>Konfirmasi Batalkan
                            </button>
                        </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (modalTolak) {
                modalTolak.addEventListener('hidden.bs.modal', function() {
                    var ta = document.getElementById('rejection-reason');
                    if (ta) {
                        ta.value = '';
                        updateCharCount(ta);
                    }
        ...
        function confirmApprove() {
            if (confirm('Yakin akan menyelesaikan agenda "{{ addslashes($data->title ?? '') }}"?')) {
