...
@section('content')
    ...
    {{-- ===================== ALERT DITOLAK (hanya jika REJECTED) ===================== --}}
    @if ($data->status == 'REJECTED' && isset($data->rejected_reason))
        <div class="alert alert-danger d-flex gap-3 mb-4 rounded-3 border-0"
            style="background: rgba(var(--bs-danger-rgb), 0.08);">
            <div class="flex-shrink-0 pt-1">
                <i class="ti ti-alert-circle fs-xl text-danger"></i>
            </div>
            <div class="flex-grow-1">
                <h6 class="fw-semibold text-danger mb-1">Agenda Ditolak</h6>
                <p class="mb-1 fs-sm">{{ $data->rejected_reason }}</p>
                <p class="mb-0 text-muted fs-xs">
                    Ditolak oleh <strong>{{ $data->approver->name ?? '-' }}</strong>
                    &bull; {{ \Carbon\Carbon::parse($data->approved_at)->translatedFormat('d M Y, H:i') }} WIB
                </p>
            </div>
        </div>
    @endif
    ...
    <div class="row g-4">
        ...
        <div class="col-xl-8">
            ...
            {{-- ---- DOKUMENTASI (hanya jika APPROVED atau COMPLETED) ---- --}}
            @if ($data->status == 'APPROVED' || $data->status == 'COMPLETED')
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between p-4 pb-3">
                        ...
                        {{-- Tombol Upload (hanya jika COMPLETED) --}}
                        {{-- @if ($data->status == 'COMPLETED')
                    ...
                    {{-- Panel Upload (collapsed by default) --}}
                    @if ($data->status == 'COMPLETED')
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div id="drop-zone"
                                                ...
                                                <div id="drop-placeholder">
                                                    ...
                                                    <p class="fw-medium text-muted mb-1 fs-sm">Klik atau seret foto ke sini
                                                    </p>
                        ...
                        @empty
                            <div class="text-center py-5">
                                ...
                                <p class="text-muted fs-sm mb-0">
                                    @if ($data->status == 'COMPLETED')
                                        Klik tombol <strong>Upload Dokumentasi</strong> di atas untuk menambahkan.
                                    @else
                                        Dokumentasi akan tersedia setelah agenda selesai.
                                    @endif
                                </p>
                            </div>
                        @endforelse
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        <div class="d-flex align-items-start gap-2">
                            <div class="flex-shrink-0 mt-1">
                                <i class="ti ti-user fs-sm text-muted"></i>
                            </div>
                            <div>
                                ...
                                <p class="fw-medium mb-0 fs-sm">
                                    ...
                                    {{-- Menggunakan Auth::id() lebih aman daripada nama --}}
                                    @if (isset($data->creator) && $data->creator->id === Auth::id())
                        ...
                        <div class="d-flex align-items-start gap-2">
                            <div class="flex-shrink-0 mt-1">
                                <i class="ti ti-calendar-plus fs-sm text-muted"></i>
                            </div>
                            <div>
                        ...
                        @if ($data->place)
                            <div class="d-flex align-items-start gap-2">
                                <div class="flex-shrink-0 mt-1">
                                    <i class="ti ti-map-pin fs-sm text-muted"></i>
                                </div>
                                <div>
